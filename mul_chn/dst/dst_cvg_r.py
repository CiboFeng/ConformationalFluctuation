...
bin=np.linspace(0,np.linalg.norm(b[0,:,1]-b[0,:,0])/2,nhi+1) ### linspace contains the end.
R=np.tile((bin[:-1]+bin[1:])/2,(nset,1))
dst=np.zeros((nset,nhi))
for i in range(nset):
    bin_idx=np.digitize(r[round(i/nset*nfr):round((i+1)/nset*nfr)].reshape(-1),bin)-1
...
for i in range(nset):
    pyw.write(f'Radial Position, Density: (The {i}-th Set)\n')
    for j in range(nhi):
        pyw.write(f'{R[i,j]} {dst[i,j]/(4*np.pi*R[i,j]**2)}\n')
    pyw.write('\n')
