...
"""Set Arguments"""
files=['dst_cvg_r/1.37sgm_0.6sgm.pyw',
       'dst_cvg_r/1.5sgm_0.82sgm.pyw',
       'dst_cvg_r/1.55sgm_1.0sgm.pyw']
figname='dst_cvg_r'
nhi=1000
nset=5
...
for i in range(len(files)):
    for j in range(nset):
        normc=norm(j)
        rgb=cmap(normc)
        ax[i].plot(r[i,j,5:],dst[i,j,5:]*unit,color=rgb,linewidth=wth)
    ax[i].autoscale()
    ax[i].minorticks_on()
    ...
    # ax[i].set_xlim(-4,84)
    if i==len(files)-1:
        ax[i].set_xlabel('Radial Position ($\\mathrm{\\AA}$)',fontsize=size)
