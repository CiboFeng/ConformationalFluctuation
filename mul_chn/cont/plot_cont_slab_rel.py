...
if not os.path.exists(figname.split('/')[0]):
    os.makedirs(figname.split('/')[0])
...
norm=colors.LogNorm(vmin=1e-2,vmax=5e0)
...
for i in range(nmd):
    for j in range(nT):
        ...
        if (i,j)==(nmd-1,0):
            ...
            ax.imshow((Pintra[-1,0]-Pintra[i,j])/Pintra[-1,0],cmap=cmap,norm=norm)
        ax.invert_yaxis()
        ax.autoscale()
        ax.minorticks_on()
...
for i in range(nmd):
    for j in range(nT):
        ...
        if (i,j)==(nmd-1,0):
            ...
            ax.imshow((Pinter[-1,0]-Pinter[i,j])/Pinter[-1,0],cmap=cmap,norm=norm)
        ax.invert_yaxis()
        ax.autoscale()
        ax.minorticks_on()
        ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
...
for i in range(nT):
    ...
    for j in range(nmd):
        ...
        if (i,j)==(0,nmd-1):
            ...
            ax.plot(np.arange(nat),(Plintra[j,i]-Plintra[-1,0])/Plintra[-1,0],color=rgb,linewidth=wth)
    ax.autoscale()
    ax.minorticks_on()
    ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
    ...
    ax.spines['right'].set_linewidth(wth)
    # ax.set_xscale('log')
    ymin=np.nanmin((Plintra-Plintra[-1:,:1])/Plintra[-1:,:1])
    ymax=np.nanmax((Plintra-Plintra[-1:,:1])/Plintra[-1:,:1])
    ...
    for j in range(nmd):
        ...
        if (i,j)==(0,nmd-1):
            ...
            ax.plot(np.arange(nat),(Psintra[j,i]-Psintra[-1,0])/Psintra[-1,0],color=rgb,linewidth=wth)
    ax.autoscale()
    ax.minorticks_on()
    ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
    ...
    ymin=np.nanmin((Psintra-Psintra[-1:,:1])/Psintra[-1:,:1])
    ymax=np.nanmax((Psintra-Psintra[-1:,:1])/Psintra[-1:,:1])
    ax.set_ylim(1.05*ymin-0.05*ymax,1.05*ymax-0.05*ymin)
    if i==nT-1:
        ax.set_xlabel(f'Sequence Distance',fontsize=size)
    ax.set_ylabel('Difference',fontsize=size)
    ax2.minorticks_on()
    ax2.tick_params(axis='y',which='major',direction='in',width=wth,length=lenmaj,labelsize=size,color='r',labelcolor='r')
    ax2.tick_params(axis='y',which='minor',direction='in',width=wth,length=lenmin,labelsize=size,color='r',labelcolor='r')
    # ax2.xaxis.set_minor_locator(AutoMinorLocator(2))
    ...
    for j in range(nmd):
        ...
        if (i,j)==(0,nmd-1):
            ...
            ax.scatter(np.arange(ntp),(plintra[j,i]-plintra[-1,0])/plintra[-1,0],color=rgb,linewidth=wth,zorder=2)
    ax.tick_params(axis='x',which='major',direction='in',width=wth,length=lenmin,labelsize=0.8*size)
    ax.tick_params(axis='x',which='minor',direction='in',width=wth,length=0,labelsize=size)
...
for i in range(nmd):
    ...
    for j in range(nT):
        ...
        if (i,j)==(nmd-1,0):
            ...
            ax.plot(np.arange(nat),(Plintra[i,j]-Plintra[-1,0])/Plintra[-1,0],color=rgb,linewidth=wth)
    ax.autoscale()
    ax.minorticks_on()
    ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
    ...
    for j in range(nT):
        ...
        if (i,j)==(nmd-1,0):
            ...
            ax.plot(np.arange(nat),(Psintra[i,j]-Psintra[-1,0])/Psintra[-1,0],color=rgb,linewidth=wth)
    ax.autoscale()
    ax.minorticks_on()
    ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
    ...
    for j in range(nT):
        ...
        if (i,j)==(nmd-1,0):
            ...
            ax.scatter(np.arange(ntp),(plintra[i,j]-plintra[-1,0])/plintra[-1,0],color=rgb,linewidth=wth,zorder=2)
    ax.tick_params(axis='x',which='major',direction='in',width=wth,length=lenmin,labelsize=0.8*size)
    ax.tick_params(axis='x',which='minor',direction='in',width=wth,length=0,labelsize=size)
    ax.tick_params(axis='y',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='y',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
...
for i in range(nT):
    ...
    for j in range(nmd):
        ...
        if (i,j)==(0,nmd-1):
            ...
            ax.plot(np.arange(nat),(Plinter[j,i]-Plinter[-1,0])/Plinter[-1,0],color=rgb,linewidth=wth)
    ax.autoscale()
    ax.minorticks_on()
    ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
    ...
    ax.spines['right'].set_linewidth(wth)
    # ax.set_xscale('log')
    ymin=np.nanmin((Plinter-Plinter[-1:,:1])/Plinter[-1:,:1])
    ymax=np.nanmax((Plinter-Plinter[-1:,:1])/Plinter[-1:,:1])
    ax.set_ylim(1.05*ymin-0.05*ymax,1.05*ymax-0.05*ymin)
    if i==nT-1:
        ax.set_xlabel(f'Residue Index',fontsize=size)
    ax.set_ylabel('Difference\nat %i K'%Ts[i],fontsize=size)
    ...
    for j in range(nmd):
        ...
        if (i,j)==(0,nmd-1):
            ...
            ax.scatter(np.arange(ntp),(plinter[j,i]-plinter[-1,0])/plinter[-1,0],color=rgb,linewidth=wth,zorder=2)
    ax.tick_params(axis='x',which='major',direction='in',width=wth,length=lenmin,labelsize=0.8*size)
    ax.tick_params(axis='x',which='minor',direction='in',width=wth,length=0,labelsize=size)
    ax.tick_params(axis='y',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='y',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
...
for i in range(nmd):
    ...
    for j in range(nT):
        ...
        if (i,j)==(nmd-1,0):
            ...
            ax.plot(np.arange(nat),(Plinter[i,j]-Plinter[-1,0])/Plinter[-1,0],color=rgb,linewidth=wth)
    ax.autoscale()
    ax.minorticks_on()
    ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
    ...
    for j in range(nT):
        ...
        if (i,j)==(nmd-1,0):
            ...
            ax.scatter(np.arange(ntp),(plinter[i,j]-plinter[-1,0])/plinter[-1,0],color=rgb,linewidth=wth,zorder=2)
    ax.tick_params(axis='x',which='major',direction='in',width=wth,length=lenmin,labelsize=0.8*size)
    ax.tick_params(axis='x',which='minor',direction='in',width=wth,length=0,labelsize=size)
    ax.tick_params(axis='y',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='y',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
