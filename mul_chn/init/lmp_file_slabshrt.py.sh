...
do
	for j in $(seq 0 1 3)
	do
		sed -i "s|^coefffile=.*|coefffile='../../max_entr_all_atom/fus_"${dirs[$i]}"/coeff_20/coeff_fus.pyw'|"  lmp_file_slabshrt.py
        	sed -i "s|^T=.*|T="${Ts[$j]}"|"  lmp_file_slabshrt.py
		python lmp_file_slabshrt.py
        	mkdir ../slabshrt_${dirs[$i]}_${Ts[$j]}
        	rm -r ../slabshrt_${dirs[$i]}_${Ts[$j]}/job_out
        	mv fus.cfg fus.par ../slabshrt_${dirs[$i]}_${Ts[$j]}
        	cd ../slabshrt_${dirs[$i]}_${Ts[$j]}
        	touch fus.sh
        	echo -e "#!/bin/bash
