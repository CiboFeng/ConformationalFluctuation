...
for i in $(seq 0 1 2)
do
	sed -i "s|^coefffile=.*|coefffile='../../max_entr_all_atom/fus_"${dirs[$i]}"/coeff_20/coeff_fus.pyw'|"  lmp_file_kin.py
	sed -i "s|^trjfile=.*|trjfile='../../max_entr_all_atom/fus_"${dirs[$i]}"/sim_20/sim_fus_0.trj'|"  lmp_file_kin.py
	sed -i 's|^nrep=.*|nrep='$nrep'|'  lmp_file_kin.py
	python lmp_file_kin.py
	for j in $(seq 0 1 $(($nrep-1)))
	do
		mkdir -p ../kin_${dirs[$i]}/rst_$j
	done
	rm -r ../kin_${dirs[$i]}/job_out
	mv fus*.cfg fus.par fus*.dat ../kin_${dirs[$i]}
	cd ../kin_${dirs[$i]}
	touch fus.sh
	echo -e "#!/bin/bash
