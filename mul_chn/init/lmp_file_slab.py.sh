...
for i in $(seq 0 1 2)
do
        sed -i "s|^coefffile=.*|coefffile='../../max_entr_all_atom/fus_"${dirs[$i]}"/coeff_20/coeff_fus.pyw'|"  lmp_file_slab.py
        python lmp_file_slab.py
        mkdir -p ../slab_${dirs[$i]}/rst
        rm -r ../slab_${dirs[$i]}/job_out
        mv fus.cfg fus.par fus.dat ../slab_${dirs[$i]}
        cd ../slab_${dirs[$i]}
        touch fus.sh
        echo -e "#!/bin/bash
