...
for i in range(nmd):
    for j in range(nT):
        for k in range(nat):
            lastnan(dmxy[i,j,:,k],100)
            lastnan(dmz[i,j,:,k],100)
        lastnan(dmmxy[i,j],100)
        lastnan(dmmz[i,j],100)
...
for i in range(nmd):
    for j in range(nT):
        par,cov=curve_fit(func,t[cut[0]:cut[1]],dmmxy[i,j,cut[0]:cut[1]])
        Parxy[i,j]=par
        par,cov=curve_fit(func,t[cut[0]:cut[1]],dmmz[i,j,cut[0]:cut[1]])
        Parz[i,j]=par
...
for i in range(nmd):
    for j in range(nT):
        ...
        for k in range(nat):
            rgb=cmap(norm(k))
            ax.plot(t,dmxy[i,j,:,k],color=rgb,linewidth=wth,alpha=0.5)
            ax.plot(t,dmz[i,j,:,k],color=rgb,linewidth=wth,alpha=0.5)
            # for l in range(nch):
            #     ax.plot(t,d[i,j,:,l,k],color=rgb,linewidth=wth,alpha=0.5)
        ax.plot(t,dmmxy[i,j,:],color='k',linewidth=wth)
        ax.plot(t,dmmz[i,j,:],color=(0.5,0.5,0.5),linewidth=wth)
        ...
        ax.set_yscale('log')
        # ax.set_ylim(40,1500)
        ax.legend(loc='upper left',fontsize=0.7*size)
...
for i in range(nT):
    ...
    for j in range(nmd):
        rgb=cmap(j/(nmd-1))
        ax.plot(t,dmmxy[j,i],color=rgb,linewidth=wth)
        ax.plot(t,dmmz[j,i],color=rgb,linestyle='--',linewidth=wth)
    ...
    for j in range(nmd):
        rgb=cmap(j/(nmd-1))
        ax.plot(t,dmmxy[j,i]/t/6,color=rgb,linewidth=wth)
        ax.plot(t,dmmz[j,i]/t/6,color=rgb,linestyle='--',linewidth=wth)
    ax.autoscale()
    ax.minorticks_on()
    ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
    ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=0,labelsize=size)
