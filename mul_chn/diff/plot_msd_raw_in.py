...
dmax=np.zeros((nmd,nT,nfr,nch,3))
for i in range(nmd):
    for j in range(nT):
...
d[d==0.0]=np.nan
d[:,:,0]=0.0
dmax[dmax==0.0]=np.nan
dmax[:,:,0]=0.0

dxy=np.sum(d[:,:,:,:,:2],axis=-1)
dz=d[:,:,:,:,-1]
dmaxxy=np.sum(dmax[:,:,:,:,:2],axis=-1)
dmaxz=dmax[:,:,:,:,-1]
...
for i in range(nmd):
    for j in range(nT):
        for k in range(nch):
            ...
            lastnan(dmaxxy[i,j,:,k],100)
            lastnan(dmaxz[i,j,:,k],100)
        lastnan(dmxy[i,j],10)
        lastnan(dmz[i,j],10)
...
for i in range(nmd):
    for j in range(nT):
        ...
        for k in range(nch):
            ...
            ax.plot(t,dmaxxy[i,j,:,k],color=rgb,linewidth=wth,alpha=0.5)
