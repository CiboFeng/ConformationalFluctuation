...
cbar.set_label('Scaled Z',fontsize=size)
...
for i in range(3):
    for j in range(nT):
        ...
        for k in range(nmd):
            rgb=cmap(k/(nmd-1))
            ax.plot(np.arange(nat),d1edge_mean[k,j,:,i],color=rgb,linewidth=wth)
        ax.autoscale()
        ax.minorticks_on()
        ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
        ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
...
for i in range(3):
    for j in range(nmd):
        ...
        for k in range(nT):
            rgb=cmap(k/(nT-1))
            ax.plot(np.arange(nat),d1edge_mean[j,k,:,i],color=rgb,linewidth=wth)
        ax.autoscale()
        ax.minorticks_on()
        ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
        ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
