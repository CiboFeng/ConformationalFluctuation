...
d=np.zeros((nmd,nT,4,nhi))
p_d=np.zeros((nmd,nT,4,nhi))
for i in range(nmd):
    for j in range(nT):
        q=pyw(f'end_dist_chk_slab/{mdls[i]}_{Ts[j]}.pyw','End-end')
        for k in range(4):
            d[i,j,k]=q[k*2]
            p_d[i,j,k]=q[k*2+1]
            if k<3:
                print(f'{mdls[i]}',f'{Ts[j]}',f'axis{k}',np.sum(p_d[i,j,k,d[i,j,k]>121.0])*(d[i,j,k,1]-d[i,j,k,0])*100)

"""Plot"""
fig,axs=plt.subplots(nT,4,figsize=(15,10))
wth=2
size=20
lenmaj=15
lenmin=8
...
for i in range(nT):
    for j in range(4):
        ...
        for k in range(nmd):
            rgb=cmap(k/(nmd-1))
            ax.plot(d[k,i,j],p_d[k,i,j],color=rgb,linewidth=wth)
        ax.autoscale()
        ax.minorticks_on()
        ...
        ax.set_yscale('log')
        if i==nT-1:
            ax.set_xlabel('End-end Distance ($\\mathrm{\\AA}$)\nof %s'%axlabels[j],fontsize=size)
        if j==0:
...
fig,axs=plt.subplots(nmd,4,figsize=(15,10))
wth=2
size=20
lenmaj=15
lenmin=8
...
for i in range(nmd):
    for j in range(4):
        ...
        for k in range(nT):
            rgb=cmap(k/(nT-1))
            ax.plot(d[i,k,j],p_d[i,k,j],color=rgb,linewidth=wth)
        ax.autoscale()
        ax.minorticks_on()
        ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
        ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
        ...
        if i==nmd-1:
            ax.set_xlabel('End-end Distance ($\\mathrm{\\AA}$)\nof %s'%axlabels[j],fontsize=size)
        if j==0:
            ax.set_ylabel('Probability Density ($\\mathrm{\\AA}^{-1}$)\nof Flu.=%.2f'%10**rg_flu[i],fontsize=size)
