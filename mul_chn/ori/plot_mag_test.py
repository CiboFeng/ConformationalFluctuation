...
p=np.zeros((nmd,nT,nhi_z,nhi_mag,3))
...
#         for k in range(nmd):
#             rgb=cmap(k/(nmd-1))
#             ax.plot(z[k,i]/((z2[k,i]-z1[k,i])/2+d[k,i]),mag_mean[k,i,:,j],color=rgb,linestyle='--',linewidth=wth)
#             ax.plot(z[k,i]/((z2[k,i]-z1[k,i])/2+d[k,i]),mag_std[k,i,:,j],color=rgb,linewidth=wth)
#         ax.autoscale()
#         ax.minorticks_on()
#         ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
#         ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
...
#         for k in range(nT):
#             rgb=cmap(k/(nT-1))
#             ax.plot(z[i,k]/((z2[i,k]-z1[i,k])/2+d[i,k]),mag_mean[i,k,:,j],color=rgb,linestyle='--',linewidth=wth)
#             ax.plot(z[i,k]/((z2[i,k]-z1[i,k])/2+d[i,k]),mag_std[i,k,:,j],color=rgb,linewidth=wth)
#         ax.autoscale()
#         ax.minorticks_on()
#         ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
#         ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
