...
#         for k in range(nmd):
#             rgb=cmap(k/(nmd-1))
#             ax.plot(tht[k,i],pedge[k,i,j]*1000,color=rgb,linewidth=wth)
#         ax.plot(tht[0,0],pedge_rand*1000,'k--',linewidth=wth,label='Random')
#         ax.autoscale()
#         ax.minorticks_on()
#         ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
#         ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
...
#         if i==0:
#             ax.set_ylabel('Probability Density\n($\\times10^{-3}$) at %s'%ylabels[j],fontsize=size)
#         if (i,j)==(0,0):
...
#         for k in range(nT):
#             rgb=cmap(k/(nT-1))
#             ax.plot(tht[i,k],pedge[i,k,j]*1000,color=rgb,linewidth=wth)
#         ax.plot(tht[0,0],pedge_rand*1000,'k--',linewidth=wth,label='Random')
#         ax.autoscale()
#         ax.minorticks_on()
#         ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
...
for i in range(nT):
    for j in range(3):
        ...
        for k in range(nmd):
            rgb=cmap(k/(nmd-1))
            ax.plot(thtabs[k,i],pabsedge[k,i,j]*100,color=rgb,linewidth=wth)
        ax.plot(thtabs[0,0],pabsedge_rand*100,'k--',linewidth=wth,label='Random')
        ax.autoscale()
        ax.minorticks_on()
        ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
        ax.tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
...
for i in range(nmd):
    for j in range(3):
        ...
        for k in range(nT):
            rgb=cmap(k/(nT-1))
            ax.plot(thtabs[i,k],pabsedge[i,k,j]*100,color=rgb,linewidth=wth)
        ax.plot(thtabs[0,0],pabsedge_rand*100,'k--',linewidth=wth,label='Random')
        ax.autoscale()
        ax.minorticks_on()
        ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
