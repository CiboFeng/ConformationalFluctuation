...
"""Set Arguments"""
file='cont_prob_mom_6.0.npy'
figname='cont_prob_6.0'
...
ax.plot(dsep,Pl[:,0],linewidth=wth)
ax.autoscale()
ax.minorticks_on()
ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
...
ax.set_ylabel('Mean Contact Probability',fontsize=size)
ax.set_xscale('log')
ax.set_yscale('log')

ax=plt.subplot(2,2,4)
ax.bar(xtp,pl[:,0],linewidth=wth,edgecolor='k')
ax.autoscale()
ax.minorticks_on()
...
ax.spines['right'].set_linewidth(wth)
ax.set_xlabel('Residue Type',fontsize=size)
ax.set_ylabel('Contact Probability',fontsize=size)
plt.xticks(xtp,res)
# ax.set_xscale('log')
# ax.set_yscale('log')
