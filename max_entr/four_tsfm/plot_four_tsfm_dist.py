...
for i in range(nmd):
    ...
    for j,k in IJ:
        normc=norm(abs(j-k))
        rgb=cmap(normc)
        ax.plot(1/f,A[i,:,j,k]/1e3,color=rgb,linewidth=wth)
    ...
    if i==0:
        ax.set_ylabel(f'Amplitude ($\\times10^3$)',fontsize=size)
    ax.set_xscale('log')
    ax.set_ylim(-0.3,6.5)
...
#         normc=norm(t[j])
#         rgb=cmap(normc)
#         ax.plot(A[i,j][IJ],color=rgb,linewidth=wth)
#     ax.plot([],[],label=labels[i])
#     ax.autoscale()
#     ax.minorticks_on()
#     ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
...
# top=ax[2].get_position().y1
# bot=ax[2].get_position().y0
# cbar_ax=fig.add_axes([0.9,bot,0.03,top-bot])
# sm=plt.cm.ScalarMappable(cmap=cmap,norm=norm)
# sm.set_array([])
# cbar=plt.colorbar(sm,cax=cbar_ax)
