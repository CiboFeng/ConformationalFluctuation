...
T0=np.zeros((nat,nat,nhi))
p_T0=np.zeros((nat,nat,nhi))
T0_mean=np.zeros((nat,nat))
T0_std=np.zeros((nat,nat))
T1=np.zeros((nat,nat,nhi))
p_T1=np.zeros((nat,nat,nhi))
T1_mean=np.zeros((nat,nat))
T1_std=np.zeros((nat,nat))
for i in range(nat):
    for j in range(i):
        seglen=[]
        segval=[]
        idxs=np.where(Qs[:,i,j]==1)[0]
        idxe=np.where(Qe[:,i,j]==1)[0]
        ...
        T0[i,j]=(edge[:-1]+edge[1:])/2
        T0[j,i]=T0[i,j]
        p_T0[i,j]=hist/np.sum(hist*(np.max(T0[i,j])-np.min(T0[i,j]))/nhi)
        p_T0[j,i]=p_T0[i,j]
        T0_mean[i,j]=np.mean(len0)
        T0_mean[j,i]=T0_mean[i,j]
        T0_std[i,j]=np.std(len0)
        T0_std[j,i]=T0_std[i,j]
        hist,edge=np.histogram(len1,bins=bin)
        # hist,edge=np.histogram(len1,bins=nhi)
        T1[i,j]=(edge[:-1]+edge[1:])/2
        T1[j,i]=T1[i,j]
        p_T1[i,j]=hist/np.sum(hist*(np.max(T1[i,j])-np.min(T1[i,j]))/nhi)
        p_T1[j,i]=p_T1[i,j]
        T1_mean[i,j]=np.mean(len1)
        T1_mean[j,i]=T1_mean[i,j]
        T1_std[i,j]=np.std(len1)
        T1_std[j,i]=T1_std[i,j]
