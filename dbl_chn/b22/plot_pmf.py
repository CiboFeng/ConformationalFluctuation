...
for i in range(nmd):
    rgb=cmap(i/(nmd-1))
    ax.plot(d[i],F[i],color=rgb,linewidth=wth)
ax.autoscale()
ax.minorticks_on()
...
ax.set_ylabel('Free Energy (kcal/mol)',fontsize=size)

ax=axs[1]
for i in range(nmd):
    rgb=cmap(i/(nmd-1))
    ax.plot(d[i],np.exp(-F[i]/kB/T)/d[i]**2,color=rgb,linewidth=wth)
ax.autoscale()
ax.minorticks_on()
ax.tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
ax.tick_params(axis='x',which='minor',direction='in',width=wth,length=0,labelsize=size)
...
ax.spines['right'].set_linewidth(wth)
ax.set_yscale('log')
ax.set_xlabel('Distance ($\\mathrm{\\AA}$)',fontsize=size)
ax.set_ylabel('Density',fontsize=size)
